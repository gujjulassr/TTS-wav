...
class MHAttn(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.WQ = self.add_weight(name="WQ", shape=(self.n_heads, input_shape[-1], self.head_size), 
                initializer=initializer, regularizer=regularizer, trainable=True)
        self.WV = self.add_weight(name="WV", shape=(self.n_heads, input_shape[-1], self.head_size), 
                initializer=initializer, regularizer=regularizer, trainable=True)
        self.WU = self.add_weight(name="WU", shape=(self.n_heads*self.head_size, input_shape[-1]), 
                initializer=initializer, regularizer=regularizer, trainable=True)
